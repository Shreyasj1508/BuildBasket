...
io.on("connection", (soc) => {
  ...
  soc.on("commission_updated", (commissionData) => {
    console.log(
      "Commission updated, broadcasting to all clients:",
      commissionData
    );
...
app.use(
  "/api/admin/users",
  require("./routes/admin/adminUserVerificationRoutes")
);
...
const PORT = process.env.PORT || 5000; // Fixed duplicate port declaration
...
server.listen(PORT, () => {
  ...
  console.log("✅ Server is running on port:", PORT); // Fixed object literal
