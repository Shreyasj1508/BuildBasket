...
const ExcelUpload = () => {
    ...
    const uploadTypes = [
        { value: 'categories', label: 'Categories', description: 'Upload product categories' },
        { value: 'products', label: 'Products', description: 'Upload products data' },
        { value: 'sellers', label: 'Sellers', description: 'Upload sellers information' },
        { value: 'customers', label: 'Customers', description: 'Upload customer data' },
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (response.status === 200 && response.data.message) {
                ...
                const totalProducts = data.totalProducts || response.data.totalProducts || 0;
                const activeProducts = data.activeProducts || response.data.activeProducts || 0;
                const inactiveProducts = data.inactiveProducts || response.data.inactiveProducts || 0;
                const pendingProducts = data.pendingProducts || response.data.pendingProducts || 0;
                
                
                if (successCount > 0 && errorCount === 0) {
                    toast.success(`✅ Successfully uploaded ${uploadType}! ${successCount} items processed.`);
                } else if (successCount > 0 && errorCount > 0) {
                    toast.success(`⚠️ Upload completed: ${successCount} successful, ${errorCount} errors. Check details below.`);
                } else if (errorCount > 0) {
                    toast.error(`❌ Upload failed: ${errorCount} errors. Check details below.`);
                } else {
                ...
                if (uploadType === 'products') {
                    setTimeout(() => {
                        toast.success(`📊 Database Status: ${totalProducts} total, ${activeProducts} active, ${inactiveProducts} inactive, ${pendingProducts} pending`);
                    }, 1000);
                }
                ...
                if (uploadType === 'products' && successCount > 0) {
                    setTimeout(() => {
                        toast.success('🔄 Please refresh the frontend page to see the new products!');
                    }, 2000);
                }
        ...
        <div className="px-2 md:px-7 py-5">
            <div className="w-full bg-white rounded-md p-6">
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800">Excel Data Upload</h2>
