        ...
        <div className='px-2 lg:px-7 pt-5'>
             <div className='flex justify-between items-center mb-6'>
                ...
                <div className='bg-gradient-to-r from-red-500 to-red-600 text-white px-4 py-2 rounded-lg shadow-lg'>
                    <span className='font-semibold'>{totalSeller} Deactive</span>
                </div>
             </div>
             <div className='w-full p-6 bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-xl'>
            
             <div className='flex justify-between items-center mb-6'>
                    <select onChange={(e) => setParPage(parseInt(e.target.value))} className='px-4 py-3 focus:border-red-500 focus:ring-2 focus:ring-red-500/20 outline-none bg-white border border-gray-300 rounded-lg text-gray-800 font-medium shadow-md hover:shadow-lg transition-all duration-300'>
                        <option value="5">Show 5</option>
                        <option value="10">Show 10</option>
                        <option value="20">Show 20</option> 
                    </select>
                    <input onChange={e => setSearchValue(e.target.value)} value={searchValue} className='px-4 py-3 focus:border-red-500 focus:ring-2 focus:ring-red-500/20 outline-none bg-white border border-gray-300 rounded-lg text-gray-800 placeholder-gray-500 shadow-md hover:shadow-lg transition-all duration-300 w-64' type="text" placeholder='Search deactive sellers...' /> 
                </div>

                <div className='relative overflow-x-auto bg-white rounded-lg shadow-lg'>
    <table className='w-full text-sm text-left text-gray-800'>
        <thead className='text-sm text-white uppercase bg-gradient-to-r from-red-500 to-red-600'>
        <tr>
        ...
        <tbody className='divide-y divide-gray-200'>
        {
                ...
                <td scope='row' className='py-4 px-4'>
                    <span className='px-3 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-800'>{ d.status }</span>
                </td>
                <td scope='row' className='py-4 px-4 text-gray-700'>{ d.shopInfo?.district }</td>
                 
                <td scope='row' className='py-4 px-4'>
                    <div className='flex justify-start items-center gap-3'>
                    <Link to={`/admin/dashboard/seller/details/${d._id}`} className='p-3 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-lg hover:shadow-lg hover:scale-105 transition-all duration-300 shadow-md'> 
